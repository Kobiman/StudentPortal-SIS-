import { Validator, Rules, Required, MinLength, MaxLength, MinNotExceedMax } from "../validator";
...
export class ProgramViewModel {
  ...
  addCreditLimit(creditLimit: any) {
  ...
  async save() {
    ...
    if (!this.validator.hasErrors) {
      ...
      BallLoader.show();
      const res = await new HttpService().post("Program/AddProgram", this.program);
      this.program = new Program();
      this.bind();
  ...
  async update() {
    ...
    if (!this.validator.hasErrors) {
      ...
      const res = await new HttpService().post("Program/UpdateProgram", this.program);
