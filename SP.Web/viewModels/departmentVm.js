...
var DepartmentVm = /** @class */ (function () {
    function DepartmentVm() {
        var _this = this;
    ...
    DepartmentVm.prototype.setupRules = function () {
        return [
            new validator_1.Rules("name", [new validator_1.Required(this.department.name), new validator_1.MinLength(this.department.name, 2)]),
            new validator_1.Rules("code", [new validator_1.Required(this.department.code)]),
        ];
    };
    DepartmentVm.prototype.save = function () {
        var _this = this;
        var rules = this.setupRules();
        var errors = this.validator.addRules(rules).validate();
        this.viewModelHelper.showErrors(errors);
        if (!this.validator.hasErrors) {
            var school = this.schools.find(function (x) { return x.name === _this.department.schoolName; });
            ...
            fetch(app_1.App.baseUri + "/api/Department/AddDepartment", options)
                .then(function (res) { return res.json(); })
                .then(function (res) {
                _this.department = new department_1.Department();
                _this.bind();
