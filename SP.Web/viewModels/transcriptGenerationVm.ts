...
export class TranscriptGenerationVm {
  ...
  getStudentResults = async (indexNumber) => {
    try {
      BallLoader.show();
      const data = await new HttpService().get(`Student/CheckResults/${indexNumber}`);
  ...
  async generateTranscript() {
    ...
    for (const result of examResults) {
      ...
      let finalY = (transcriptFile as any).previousAutoTable.finalY;
  ...
  setBody(result) {
   ...
   degreeClassification(transcriptFile: typeof dummyJspdf){
  ...
  async generateStatementOfResult() {
    const offset = 5;
    ...
    let previousResults = [];
    for (const result of examResults) {
      let finalY = (statementOfResult as any).previousAutoTable.finalY;
      previousResults.push(... result.data);
      result.semester.substring(8);
