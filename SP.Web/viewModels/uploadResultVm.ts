...
export class UploadResultViewModel {
  ...
  studentsEnteryLevel: any = {};
  lookupVm: any;
  names: any;
  updateCourseCommand: Command = new Command();
  hasErrors: boolean = false;
  grades: any[];
  resultSummary: { grade: any; total: any }[];
  academicYear: string;
  semester: string;
  ...
  async save() {
    if (this.hasErrors) {
      ...
      let results = this.studentResults.map((x) => ({
        ...
        mountedCourseId: names2[names2.length - 1].substring(
          0,
          names2[names2.length - 1].length - 4
        ),
        level: x.Level,
        grade: x.Grade,
  ...
  getProgram(mountedCourseId: any) {
    let mountedCourse = this.courses.find(
      (x) => x.mountedCourseId === mountedCourseId
    );
    this.program = this.commonService.programs.find(
      (x) => x.programId === mountedCourse.programId
    );
  }
  ...
  getStudentsEnteryLevel = async () => {
    try {
      BallLoader.show();
      const data = await this.httpService.get("Student/GetStudentsEnteryLevel");
  ...
  getEnrollmentOptions() {
    let enrollmentOptions = this.commonService.lookups.filter(
      (x) => x.type == "ENROLLMENT OPTION"
    );
    return enrollmentOptions;
  }

  getSemesters() {
    let semesters = this.commonService.lookups.filter(
      (x) => x.type == "SEMESTER"
    );
    return semesters;
  }
  ...
  getAcademicYears() {
    let academicYears = this.commonService.lookups.filter(
  ...
  public async getMountedCourses(academicYear, semester, enrollmentOption) {
    BallLoader.show();
    var request = {
      academicYear: academicYear,
      semester: semester,
      enrollmentOption: enrollmentOption,
      lecturerId: this.commonService.lecturers.find(
        (x) => x.staffId === App.user.profile.userId
      ).lecturerId,
    };
    let result = await this.httpService.post(
      "Department/GetMountedCoursesForResultsUpload",
      request
    );
  ...
  async download_csv(
    ...
    let result = await this.httpService.post(
      "Department/GetRegisteredStudentsList",
      request
    );
  ...
  bindData = () => {
    this.mountedCourseRequest =
      this.viewModelHelper.addPropertyChangeNotification(
        this.mountedCourseRequest
      );
  };
  getCourse = () => {
        ...
        Papa.parse(csv, {
          ...
          complete: (results) => {
            ...
            if (results.data.length < registeredStudents.length) {
              ...
              this.studentResults = this.formartResults(
                results,
                registeredStudents
              );
              new ResultRow().render(this.studentResults);
  ...
  private formart(groupedResults) {
  ...
  generateResultsSheet() {
    ...
    doc.text(
      "Summary of Results",
      14,
      (doc as any).previousAutoTable.finalY + 18
    );
    autoTable(doc, {
    ...
    doc.text(
      `Uploaded by: ${
        this.commonService.lecturers.find(
          (x) => x.staffId === App.user.profile.UserId
        ).name
      }`,
      14,
      (doc as any).previousAutoTable.finalY + 15
    );
    doc.setFontSize(8);
    var current = new Date();
    doc.text(
      `Date Issued(d/m/y): ${current.toLocaleString()}`,
      14,
  ...
  private createRequest(names, file) {
  ...
  private formartResults(results, registeredStudents) {
    for (const r of results.data) {
      let registeredStudent = registeredStudents.find(
        (x) => x.indexnumber === r.IndexNumber
      );
      ...
      let student = this.commonService.students.find(
        (x) => x.indexNumber === r.IndexNumber
      );
